...
describe('get correct file name given a url path', () => {
  ...
  test('should return the file name from a URL with query parameters', () => {
    const result = getFileNameFromUrl('http://example.com/path/to/file.txt?param=value')
    expect(result).toBe('file.txt')
  })
  ...
  test('should return an empty string if the URL has no file name', () => {
    const result = getFileNameFromUrl('http://example.com/path/to/')
    expect(result).toBe('')
  })
